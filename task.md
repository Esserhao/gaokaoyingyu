...
## 2026-09-22（晚五）· 补语篇类知识节点 + 短文改错方法卡

**用户**：先问「语篇类你打算写哪些知识点？还有哪些知识点或做题方法技巧需要补充？」，
随后拍板范围＝**A 组语篇·阅读 6 节点 + 短文改错方法卡**，并同意**一并补语法根节点
「句子成分与基本句型」**（B 组语篇·连贯 4 节点留后）。

**调研结论（本次动手前的数据依据）**
- 原 50 节点构成：语法·句法 25 / 语法·词法 13 / 词汇·用法 8 / 写作·表达 4 —— **语篇 0**。
- 关键发现：`explanation.summary` 里「｜」之前那一段是**受控的题型标签**，240/240 道
  阅读题都有。按此可算出真实考频，且核实过 **`tools/build_kb_freq.js` 里这些信号
  出现次数为 0 —— 从未被使用**，是现成的富矿（无需新数据源）。
- 七选五 40/80 题只标「七选五」、其余是内容描述，**职能（段首/段中/段末）无法从语料
  反推**，故 B 组（语篇·连贯）将来只能用「题型覆盖」口径，绝不编次数。

**① 新增 7 个知识节点（tools/build_knowledge.py）**
- `CATEGORY_ORDER` 增「语篇·阅读」（排在 词汇·用法 与 写作·表达 之间）。
- 6 个语篇·阅读：细节理解题 / 推理判断题 / 词句猜测题 / 主旨大意题 / 标题归纳题 /
  观点态度与写作意图。1 个语法·句法：句子成分与基本句型（七种成分 + 五种基本句型
  —— 25 个句法节点里唯一缺的基础，也是语法填空「先分型」的前提）。
- 每节点必备 12 字段齐全、examples ≥3、commonErrors ≥2、confusionPoints ≥2。
- POINTS 补丁表同步补 7 条「一句话 rule + 分点（每点配例 + 中文点题）」，共 **29 个分点**，
  守用户既有的「要点分行、逐条举例」要求。**例句与示例句全部自拟，source 如实标
  「自编示例」/「自编练习」，不假托真题或教材出处。**

**② 56 道节点题（tools/kb_questions_e.py，新增并注册进 QUESTION_BATCHES）**
7 节点 × 8 道 = 56 道（题池校验是硬门槛：每节点 <8 道直接不生成文件）。
每节点 4 个变体 × 2 道，choice 为主、语法节点混 fill。全部 verified=False、未人核。
题池总量 400 → **456**。

**③ 给新节点接真题考频（tools/build_kb_freq.js）**
- PATTERNS 增 6 条阅读题型信号（含标签变体：词句/词义猜测题、标题归纳/判断题、
  段落主旨题、写作意图/手法题）与 1 条 `句子成分|基本句型`。
- 实测考频：**细节理解题 112 次 · 15 卷（全站第一，原第一是名词 56 次）**、推理判断 74·15、
  词句猜测 14·14、主旨大意 14·12、标题归纳 5·5、观点态度与写作意图 4·3；
  句子成分与基本句型如实为 0 → 界面标「真题未直接考查」。
- 刻意**不用** `/句子结构/` —— 那会命中短文改错里 `knowledgeNode=句子结构` 的祈使句
  改错点，与「祈使句与感叹句」节点重复计数。

**④ 短文改错方法卡（data/methods/index.js，手写文件直接改）**
- 这是七张卡里唯一的**结构性缺口**：全国甲 / 乙卷仍在考该题型，本题库 **7 套卷**
  （gk2021-jia/yi、gk2022-jia/yi、gk2023-jia/yi、gk2024-jia）含 proofreading 节、
  且与语法填空并存，但此前点进方法卡是空的。
- 新增 5 步（PR-SPLIT 通读定型 / PR-TYPE 按类排查 / PR-LOCATE 定位到词 /
  PR-SYMBOL 规范改法 / PR-COUNT 数够十处），每步含 动作 + 禁止 + 回退，与既有卡同构。
- **step.id 全部新前缀 `proofreading:PR-*`，未改动任何已有 step.id**（step.id 即错题
  errStage 词表，改 id 断历史归因）。归因 chips 是动态生成的（`stageChips` 按
  sectionKey 查卡），故短文改错错题自动获得 5 个 chips、`#/methods/proofreading`
  路由也自动可用。note 与三处注释同步（客观题五张 → 六张）。

**⑤ 验证（66 路由快照 + 13 个探针）**
- 快照：65 → **66**（+ 细节理解题 详情页 / + #/methods/proofreading；顺手把快照里
  指向不存在节点的旧路由 `细节理解` 改成真节点 `细节理解题`）。**仅 5 条路由变化**
  （#/knowledge +157、#/learn +6809、#/knowledge-graph ±2 与语法·句法簇 +978、
  #/methods +239），无空页、无错误页。
- 抽查真实渲染（不只看 diff）：#/learn 分组 **26/13/8/6/4 = 57**，语篇·阅读 6 行考频
  分别 112·15 / 74·15 / 14·14 / 14·12 / 5·5 / 4·3，句子成分标「未直接考查」；
  57 个节点**每个都有考频徽标**；详情页 4 条要点各带例句与中文点题；方法卡 8 张、
  链接含 proofreading。
- **13 个探针全绿**：uxprobe 18/18、kbprobe **20/20**（新增 4 条语篇断言）、
  methods-probe 9/9、layoutprobe 34/34、mobileprobe 13 路由零溢出、todayprobe 22/22、
  deskprobe 18/18、writingprobe、listenprobe 6/6、scoreprobe 5/5、
  unitdiag-probe 6/6、printprobe 7/7、shprobe 6/6。
- 基线重设 `snap-a.json`（留档 `snap-rev-pre-yupian.json`）。

**探针过时两处（页面正确，改的是探针）**
1. methods-probe：M1 期望 7 张卡 → 8；M4 原以 `proofreading` 作「主观题不出 chips」的
   样本，但短文改错本就是客观题、现在有卡就会出 chips → 改用真正无卡的合成 sectionKey
   守 `!s → ''` 分支，并新增「短文改错 5 步 chips 且顺序正确」断言；M5 编辑器数 2→3。
2. unitdiag-probe：U1/U2/U5 写死「语法·句法 25 个」，给该单元加了 1 个节点后三处一起
   误报。**改为从 KB 现算**（`unitCounts['语法·句法']`），以后加节点不会再误报。
3. uxprobe：知识台阶节点数写死 50 → 改为与 `window.__KB__.length` 比对。

**留后**：B 组语篇·连贯 4 节点（指代与词汇衔接 / 逻辑连接与过渡 / 段落组织与句际职能 /
语篇类型与文体意识）——需先想清楚「题型覆盖」口径在界面上的措辞（现有 `kind:'section'`
的标签写死「套写作卷」，用于语篇会不准，要么改 label 字段要么换措辞）；
听力知识节点 0 个；整卷答题顺序与时间分配方法卡；长难句拆解独立成卡；
听力解析 140 道；5 套卷补 answerSource；其余页面减负。
