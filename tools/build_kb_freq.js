...
const PATTERNS = {
  ...
  // ---- 语篇·阅读（2026-09-22 新增节点）----
  // 解析摘要里「｜」之前那一段是受控的题型标签（240/240 道阅读题都有），
  // 这批信号此前完全没被使用过。正则覆盖标签的变体写法与括号后缀。
  '细节理解题': [/细节理解题/],
  '推理判断题': [/推理判断题/],
  '词句猜测题': [/词[句义]猜测题/],
  '主旨大意题': [/主旨大意题|段落主旨题/],
  '标题归纳题': [/标题归纳题|标题判断题/],
  '观点态度与写作意图': [/观点态度题|写作意图题|写作手法题/],
  // 语法根节点：语料里几乎没有对应标签，如实统计（预期为 0 → 界面标「真题未直接考查」）。
  // 刻意不用 /句子结构/ —— 那会命中短文改错里 knowledgeNode=句子结构 的祈使句改错点，
  // 与「祈使句与感叹句」节点重复计数。
  '句子成分与基本句型': [/句子成分|基本句型/],
};
